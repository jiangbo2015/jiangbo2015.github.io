...
layer.open = (config) => {
        ...
        ReactDOM.render( < Modal visible title = "Title"
            transitionName = "zoom"
            ...
            className = { classnames(prefixCls, className, [`${prefixCls}-${index}`]) } > <div className={`${prefixCls}-body-wrapper`} style={{ maxHeight: document.body.clientHeight - 256 }}>
        {content}
      </div> < /Modal>, div)
            return index
        }
